...
void compile_program_to_binary(IRProgram *program, X64instruct **out_insns, int *out_count, int **out_starts,
			       char ***out_names, int *out_function_count)
{
	X64instruct *merged = NULL;
...
{
	char cmd[512];
	snprintf(cmd, sizeof cmd, "cc %s -o %s", obj_path, output_path);
	system(cmd);
}
