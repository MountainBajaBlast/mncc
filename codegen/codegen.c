     ...
     for (int i = 0; i < count; i++) {
           switch (inst[i].op) {
               ...
               case ASM_DIV:
                  ...
                  fprintf(out, "  msub w1, w%d, w%d, w%d\n", inst[i].reg, inst[i].src2, inst[i].src1);
                  break;
                case ASM_RET: 
                     if (inst[i].is_imm) {
...
void compile_to_binary(IRGraph *graph, const char *output_name) {
      if (!graph || !graph->head) return;
       ...
       while (current_block != NULL) {
         ...
         while (quad != NULL) {
            total_instr++;
            if (quad == current_block->tail) break;
            quad = quad->next;
         }
         current_block = current_block->next_block;
       }

      RISCinstruct *asm_code = malloc((total_instr + graph->block_count) * sizeof(RISCinstruct));
           if (!asm_code) {
    ...
    while (current_block != NULL) {
        ...
        while (quad != NULL) {
            ...
            switch (quad->operation) {
                case ADD:    asm_code[idx].op = ASM_ADD; break;
                case SUB:    asm_code[idx].op = ASM_SUB; break;
                case MULT:   asm_code[idx].op = ASM_MUL; break;
                case DIV:    asm_code[idx].op = ASM_DIV; break;
                case ASSIGN: asm_code[idx].op = ASM_MOV; break;
                case RET:    asm_code[idx].op = ASM_RET; break;
                case CMP: asm_code[idx].op = ASM_CMP; break;
                case JMP: asm_code[idx].op = ASM_JMP;
                          if (current_block->jmp_target)
                              asm_code[idx].target = current_block->jmp_target->id_block;
                          emitted_jump = 1;
                          break;
                case JE: asm_code[idx].op = ASM_JE;
                         if (current_block->true_target_je)
                             asm_code[idx].target = current_block->true_target_je->id_block;
                         if (current_block->false_target_je)
                             asm_code[idx].target2 = current_block->false_target_je->id_block;
                         emitted_jump = 1;
                         break;
                default: break;
            }
            ...
            if (quad->arg1) {
                if (quad->arg1->type == NUM) {
                    ...
                    asm_code[idx].src1 = (int)quad->arg1->val.val_int;
                } else {
                    asm_code[idx].src1 = quad->arg1->val.tmp_index;
                }
            }
            ...
            if (quad->arg2) {
                if (quad->arg2->type == NUM) {
                    asm_code[idx].is_imm = 1;
                    asm_code[idx].src2 = (int)quad->arg2->val.val_int;
                } else {
                    asm_code[idx].src2 = quad->arg2->val.tmp_index;
                }
            }
...
FILE *asm_file = fopen("temp_output.s", "w");
    ...
    char cmd[512];
    snprintf(cmd, sizeof(cmd), "clang -arch arm64 temp_output.s -o %s", output_name);
