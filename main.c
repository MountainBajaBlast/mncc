...
int main(int argc, char *argv[])
{
    ...
    QuadLiveness *quad_liveness_array = calloc(total_quads, sizeof(QuadLiveness));
    ...
    BlockLiveness *block_liveness_array = calloc(block_count, sizeof(BlockLiveness));

    compute_local_liveness(ssa_graph, quad_liveness_array, block_liveness_array);
    compute_global_liveness(ssa_graph, quad_liveness_array, block_liveness_array);

    CollectIntervals *cointerval = create_intervals(ssa_graph->reg_count);

    build_intervals(ssa_graph, quad_liveness_array, block_liveness_array, cointerval);
    sort_intervals(cointerval);

    PhysRegTrack reg_track;
    for (int i = 0; i < 31; i++) reg_track.phys_reg[i] = -1;
