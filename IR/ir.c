...
void free_phis(BasicBlock* block) {
	...
	void bb_add_predecessor(BasicBlock* block,   BasicBlock* pred) {
		 ...
		 block->predecessors  = realloc(block->predecessors, block->pred_cap * sizeof(BasicBlock*));
	     }
	...
	DominatorTree* create_dominator_tree(IRGraph* graph) {
		 ...
		 BasicBlock** bucket = calloc(graph->block_count + 1, sizeof(BasicBlock*));
		 BasicBlock** block_by_dfn = calloc(graph->block_count + 1, sizeof(BasicBlock*)); 
	...
	void bb_add_successor(BasicBlock* block,   BasicBlock* succ) {
		 ...
		 block->successors  = realloc(block->successors, block->succ_cap * sizeof(BasicBlock*));
	     }
	...
	static void dfs_pass(BasicBlock* node, DominatorTree* domtree, int* next_num) {
		...
		if (domtree->dfn[succ->id_block] == 0) {    
		    domtree->parent[*next_num] = domtree->dfn[node->id_block];
	...
	void build_dominators(DominatorTree* domtree) {
		...
		while (v != 0) {
		    int u = dsu_eval(domtree, v);   
		    domtree->idom[v] = (domtree->semi[u] < domtree->semi[v]) ? u : p;
		    v = bucket_next[v];
		}
		bucket_head[p] = 0;
	    }
	...
	BasicBlock* get_idom(DominatorTree* domtree, BasicBlock* block) {
	       return domtree->block_by_dfn[domtree->idom[domtree->dfn[block->id_block]]];
	}
	...
	static void ssa_rename(IRGraph* graph, BasicBlock* block,
			       int** versions, int* top, int* cap,
			       int* pushed, int* pushed_idx, int regs) {
		...
		int already = 0;
		for (int k = 0; k < i; k++)
		    if (block->phis[k].orig_var == var) { already = 1; break; }
		if (already) continue;
		...
		for (int j = 0; j < succ->phi_count; j++) {
		    if (succ->phis[j].from_label != block->id_block) continue;
...
void out_of_ssa(IRGraph* graph) {
    for (int i = 0; i < graph->block_count; i++) {
        ...
        for (int p = 0; p < block->pred_count; p++) {
            ...
            for (int k = 0; k < n; k++)
                insert_before_terminator(pred, create_quadriple(
...
void dump_graph(IRGraph* graph) {
    static const char* opnames[] = {"RET","DIV","MULT","ADD","SUB","ASSIGN","JMP","JE","PHIN","CMP","TERMINATOR_NONE"};
    ...
    for (BasicBlock* b = graph->head; b; b = b->next_block) {
        ...
        for (int i = 0; i < b->succ_count; i++) printf(" %d", b->successors[i]->id_block);
        printf(" pred:");
        for (int i = 0; i < b->pred_count; i++) printf(" %d", b->predecessors[i]->id_block);
        printf(" phi:");
        for (int i = 0; i < b->phi_count; i++) printf(" [%d<-%d r=%d o=%d]", b->phis[i].reg_index, b->phis[i].from_label, b->phis[i].result_reg, b->phis[i].orig_var);
        printf("\n");
        for (Quadriple* q = b->head; q; q = q->next) {
            const char* t1 = q->arg1 ? (q->arg1->type==VIR_REG?"vr":q->arg1->type==NUM?"num":q->arg1->type==REG_PHYS?"phys":"?") : "null";
            const char* t2 = q->arg2 ? (q->arg2->type==VIR_REG?"vr":q->arg2->type==NUM?"num":q->arg2->type==REG_PHYS?"phys":"?") : "null";
            const char* tr = q->result ? (q->result->type==VIR_REG?"vr":q->result->type==NUM?"num":q->result->type==REG_PHYS?"phys":"?") : "null";
            long long v1 = q->arg1 ? (q->arg1->type==NUM ? q->arg1->val.val_int : q->arg1->val.tmp_index) : -1;
            long long v2 = q->arg2 ? (q->arg2->type==NUM ? q->arg2->val.val_int : q->arg2->val.tmp_index) : -1;
            long long vr = q->result ? (q->result->type==NUM ? q->result->val.val_int : q->result->val.tmp_index) : -1;
    ...
    switch(tree->type) {
        ...
        case ND_BINARY_OP: {
            ...
            switch (tree->BinaryOp.op) {
               ...
               case TK_SUBTRACTION: op = SUB;  break;
               case TK_MULTYPLY:    op = MULT; break;
               case TK_DIVISION:    op = DIV;  break;
               case TK_CMP:         op = CMP;  break;
              default:
