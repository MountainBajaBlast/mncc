    ...
    while (scan != NULL) {
        ...
        if (scan->operation == ASSIGN && scan->result != NULL) {
            ...
            if (op->type == VAR && scan->result->type == VAR &&
                scan->result->val.var_name != NULL && op->val.var_name != NULL &&
                strcmp(op->val.var_name, scan->result->val.var_name) == 0) {
                return (scan->arg1 != NULL && scan->arg1->type == NUM && scan->arg1->val.val_int == 0);
            }
        }
        scan = scan->prev;
...
int if_two_nums_cmp(Quadriple* curr) {
    if (curr->operation != CMP) return 0;
            ...
            for (int j = 0; j < block->phi_count; j++) {
                ...
                while (scan != NULL) {
                    if (scan->operation == ASSIGN &&
                        scan->result != NULL && scan->result->type == VIR_REG &&
                        scan->result->val.tmp_index == src_reg &&
...
static void run_copy_propagation(BasicBlock* block) {
    if (block == NULL || block->head == NULL) return;
    ...
    do {
        ...
        Quadriple* curr = block->head;
        while (curr != NULL) {
            if (curr->operation == ASSIGN &&
                curr->result != NULL && curr->result->type == VIR_REG &&
...
void fold_conditional_branch(BasicBlock* block) {
    if (block == NULL || block->terminator != JE) return;

    Quadriple* je = block->tail;
    if (je == NULL || je->operation != JE) return;

    Operand* cond = je->arg1;
    if (cond == NULL || cond->type != VIR_REG) return;
...
void remove_unreachable_blocks(IRGraph* graph) {
    if (graph == NULL || graph->head == NULL) return;
...
void run_phis_dce(BasicBlock* block, int* use_tab) {
      if (block == NULL || block->phi_count == 0) return;
